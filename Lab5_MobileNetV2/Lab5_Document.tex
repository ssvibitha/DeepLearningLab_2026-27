...
\vspace{5mm}
Parameters are values learned automatically by the model during training. Ex: weights and bias of neurons. 
Hyperparameters are set by the user before or during training. Ex: learning rate, batch size, number of layers, dropout rate.
Parameters directly determine the model's predictions, while hyperparameters control how the model learns.
\vspace{3mm}
...
If weights are too small, activations or gradients may shrink through layers causing vanishing gradients.
If too large, activations or gradients can become extremely large causing exploding gradients.
Good initialization also helps break symmetry between neurons.
Hence, good initialization helps maintain stable gradients through deep layers.
\vspace{3mm}

\item \textbf{Why can zero initialization be problematic for neural networks?}

\vspace{5mm}
If all weights are initialized to zero, neurons in the same layer produce identical outputs.
Their gradients are also identical during backpropagation.
This problem is known as symmetry problem.
As a result, multiple neurons fail to learn different features.
Hence, weights are usually initialized with small random values rather than all zeros, while biases can generally be initialized to zero.
\vspace{3mm}

\item \textbf{Compare Xavier and He initialization.}

\vspace{5mm}
Xavier initialization is mainly used for activation functions such as sigmoid and tanh.
It maintains similar variance between input and output layers. 
Its variance is approximately based on both the number of input and output connections.
He initialization is mainly used for ReLU and its variants, where roughly half the activations may become zero. 
He uses a larger variance, commonly to compensate for this effect. Thus, He is generally preferred for ReLU-based deep networks.
\vspace{3mm}

\item \textbf{How can training and validation curves identify overfitting?}

\vspace{5mm}
In an ideal model, training and validation loss decreases. During overfitting, training error decreases and validation error increases.
This indicates that the model is learning the training data too specifically and is not generalizing well to unseen data.
\vspace{3mm}
...
This prevents the network from depending too strongly on particular neurons or specific feature combinations. 
When random neurons are turned off, the network is forced to learn more robust and independent features. 
During testing, dropout is disabled and the complete network is used. Thus, Dropout acts as a regularization technique that improves generalization.
\vspace{3mm}

\item \textbf{What is the purpose of Batch Normalization?}

\vspace{5mm}
Batch Normalization normalizes the activations of a layer using the mean and variance calculated from a mini-batch. 
It produces standardized activations and then uses learnable parameters $\gamma$ and $\beta$ to restore useful scale and shift. 
This helps make optimization more stable and allows the use of higher learning rates. 
It can also act as a mild regularizer because batch statistics vary between mini-batches. 
Therefore, it helps neural networks train more reliably and efficiently.
\vspace{3mm}
...
\textbf{Scale and Shift Transformation}
\begin{equation}
y_i = \gamma \hat{x}_i + \beta
\end{equation}
...
\textbf{where,}
\begin{itemize}
    \item $\epsilon$ - A small numerical constant to prevent division by zero.
    \item $\gamma, \beta$ - Learnable parameters updated during training via backpropagation.
\end{itemize}
\vspace{3mm}
...
\vspace{5mm}
In Batch Normalization, $\gamma$ is the learnable scale parameter and $\beta$ is the learnable shift parameter.
Gamma controls how much the normalized activation is stretched or compressed, while beta controls how much it is shifted. 
These parameters allow the network to learn an appropriate distribution instead of being forced to use only standardized values.
\vspace{3mm}

\item \textbf{Compare SGD, Momentum, RMSProp and Adam.}

\vspace{5mm}
SGD updates weights using the gradient of the current mini-batch. 
Momentum adds a velocity term that accumulates previous gradients helping accelerate movement in consistent directions and reduce oscillations. 
RMSProp adapts the learning rate for each parameter using a moving average of squared gradients. 
Adam combines first moment from momentum and second moment from RMSProp giving adaptive updates for each parameter. 
Hence, Adam is more suitanle for faster training.
\vspace{3mm}
...
If it is too large, the optimizer may overshoot the minimum instead of moving toward it. 
The loss may oscillate, diverge, or become unstable, and the model may fail to converge.
\vspace{3mm}

\item \textbf{What happens when the learning rate is too small?}

\vspace{5mm}
When the learning rate is too small, parameter updates become very small. 
The model may require a very large number of iterations for convergence.
It can also appear as if the loss has stopped improving because progress is extremely small.
\vspace{3mm}

\item \textbf{What is the effect of increasing batch size?}

\vspace{5mm}
Increasing batch size means that more training examples are used to calculate each gradient update. 
This generally produces a more stable and less noisy gradient and improves computational efficiency on GPUs.
The number of updates per epoch also decreases when batch size increases.
However, very large batches may require more memory and can reduces noise that can improve optimization and generalization. 
\vspace{3mm}

\item \textbf{Explain stride and padding.}

\vspace{5mm}
Stride specifies how far the convolution filter moves across the input at each step. 
A larger stride reduces the spatial dimensions of the output. 
Padding adds extra pixels usually zeros around the input boundary before convolution.
It allows control over output size and helps preserve information near the edges. 
\vspace{3mm}

\item \textbf{Why is MobileNetV2 computationally efficient?}

\vspace{5mm}
MobileNetV2 uses depthwise separable convolutions which require significantly fewer computations than standard convolutions. 
It also uses an inverted residual structure with linear bottlenecks where computation is performed efficiently in an expanded intermediate representation and then compressed. 
Depthwise convolution handles spatial filtering separately for each channel reducing parameter count and computation. 
These design choices make MobileNetV2 suitable for devices with limited computational resources. 
\vspace{3mm}

\item \textbf{What is depthwise separable convolution?}

\vspace{5mm}
A depthwise separable convolution divides a standard convolution into two operations: depthwise convolution and pointwise convolution. 
Depthwise convolution applies one spatial filter independently to each input channel, while pointwise convolution uses a 1 x 1 convolution to combine information across channels.
This reduces the number of parameters and operations. Hence, it makes CNNs computationally cheaper while retaining useful feature extraction capability.
\vspace{3mm}

\item \textbf{What is transfer learning?}

\vspace{5mm}
Transfer learning is the process of using knowledge learned by a model on one dataset or task for another related task. 
A pretrained CNN, may have already learned general features such as edges, textures, and shapes from a large dataset. 
These learned weights can then be reused for a new, usually smaller dataset. This reduces training time and the amount of data required. 
Hence, it is used when the target dataset is limited.
\vspace{3mm}

\item \textbf{Differentiate feature extraction and fine-tuning.}

\vspace{5mm}
In feature extraction, the pretrained model's earlier layers are kept frozen and only a new classification head or selected layers are trained. 
The pretrained features are reused without changing their weights. 
In fine-tuning, some or all pretrained layers are unfrozen and their weights are updated using the new dataset. 
Fine-tuning allows the model to adapt its learned representations to the target task.
\vspace{3mm}

\item \textbf{Why is a smaller learning rate generally used during fine-tuning?}

\vspace{5mm}
During fine-tuning, a pretrained network which already contains useful learned features is used. 
Using a large learning rate can cause large weight updates that destroy these useful representations. This is known as catastrophic forgetting. 
A smaller learning rate allows the pretrained weights to change gradually according to the new task. 
This provides controlled adaptation while preserving previously learned knowledge.
\vspace{3mm}

\item \textbf{Why is K-Fold Cross-Validation useful for hyperparameter selection?}

\vspace{5mm}
K-Fold Cross-Validation divides the training data into K subsets called folds. The model is trained on K-1 folds and validated on the remaining fold.
This process is repeated K times.
The validation results are then averaged to estimate how well a particular hyperparameter configuration generalizes. 
This reduces dependence on one particular train-validation split. This acts as a more reliable method to select hyperparameters.
\vspace{3mm}

\item \textbf{Why must the test set remain untouched during tuning?}

\vspace{5mm}
The test set must remain untouched during tuning to prevent data leakage and ensure an unbiased evaluation of how the model performs on truly unseen real-world data.
Using testing set for training may lead to overfitting as the test data may be used to influence model predictions.  
\vspace{3mm}
...
\vspace{5mm}
The mean describes the average performance of a model across different folds, runs, or experiments. The standard deviation describes how much that performance varies around the mean. 
A high mean with a large standard deviation indicates that performance may be unstable across different data splits or runs. 
Hence reporting both mean and standard deviation helps to make experimental results more reliable and easier to interpret.
\vspace{3mm}

\item \textbf{Is the highest validation accuracy always sufficient to select a model?}

\vspace{5mm}
No, the highest validation accuracy is not always sufficient to select a model. 
If the validation dataset has unequal data for different classes, the model can repeatedly predict majority class. Hence the model may misclassify target class due to class imbalance.
Also the model may overfit if the same validation dataset is used to tune hyperparameters repeatedly.
